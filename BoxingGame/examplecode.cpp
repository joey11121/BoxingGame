...
char content[100]; //存放輸出內容
const int width = 800, height = 500; // 遊戲畫面大小
RECT rect;
const int step = 27;
const int jump = 300; 
const int interval = 120; //FPS
const int total_health = 50; //血量
int mirror = 0;
BITMAP bm;
void debug(HDC& hDC, HWND& hWnd)
{
	...
	HFONT hFont = (HFONT)CreateFont(100, 0, 0, 0,
		...
		DEFAULT_PITCH | FF_DONTCARE, "粗體字"); //調整字型
...
public:
	int x, y, l, r, top, bottom; //人物位置判定
	int dir; // 人物朝向
	HBITMAP* hbm[13][2]; //bitmap store，分朝左的bitmap和朝右的bitmap
	int num[13];
	int index, key; //關鍵偵動畫，key就是人物的狀態。
	int angry = 0;
...
int role_index[2], choice[2]; // 角色選擇
int health[2] = { 0 }; // 人物血量
int power[2] = { 0 };  // 人物怒氣

int gamestatus = 0; //初始game status
...
void SetGameStatus(int val)
{
	...
	case 3:
		...
		role1.dir = 0;  //左
		role2.dir = 1; //右
		role1.x = client.cx / 10; //role 1 x軸
		role2.x = client.cx * 4 / 5 + client.cx / 10; //role 1 y 軸
...
long WINAPI WndProc(HWND hWnd, UINT iMessage, UINT wParam, LONG lParam)
{
	...
	switch (iMessage)
	{
	case WM_KEYDOWN: //任意一個鍵，因為第二位玩家使用上下左右控制。
		switch (gamestatus)
		{
			/*
				当前的游戏状态
		...
		case 2:
			...
			case VK_LEFT:
				if (choice[1] != -1) break;
				role_index[1] = (role_index[1] + 4) % 5; //第二個人選擇的角色，相當於往左一個
				break;
			case VK_RIGHT:
				if (choice[1] != -1) break;
		...
		case 3: //role 2的格鬥畫面
			...
			case VK_LEFT:
				...
				if (GetKeyState('3') < 0) //我不知道GetKeyState是什麼，key按下的狀態如何？
				{
					role2.dir = 0;
					role2.set(8);
				}
		...
		break;
	case WM_CHAR: //字符，因為第一位玩家使用WASD控制
		switch (gamestatus)
		{
		case 2:
			switch (wParam)
